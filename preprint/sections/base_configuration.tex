...
\begin{figure}[ht]
\centering
\resizebox{1.0\textwidth}{!}{\input{figures/19-pseudolines}}
...
\input{generated/seed19_normalized_data}
...
Section~\ref{sec:certified-package}) uses interval arithmetic on closed
...
this order is compatible with the first row $\mathrm{O}[0]$ of (\ref{eq:seed19-omatrix}),
...
Section~\ref{sec:certified-package} (though in principle this could be done by hand).
...
computer-assisted verification (Section~\ref{sec:certified-package}) carries out
...
computer-assisted verification (Section~\ref{sec:certified-package}),
